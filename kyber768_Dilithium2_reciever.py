...
"""
PQC IoT Receiver
Crypto: Kyber768 + Dilithium/Falcon
Receives encrypted sensor data from Raspberry Pi sender
"""
import oqs
...
class PQCReceiver:
    ...
    def generate_keypairs(self):
        ...
        print("\n[RECEIVER] Generating Kyber768 keypair...")
        self.kyber_public_key = self.kem.generate_keypair()
        self.kyber_secret_key = self.kem.export_secret_key()
        print(f"[RECEIVER] ✓ Kyber768 keypair generated ({len(self.kyber_public_key)} bytes)")
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            if msg.topic == TOPIC_ENCRYPTED_DATA:
                ...
                print(f"{'='*80}")
                print(f"  ✓ Signature: VERIFIED ({t_sig_ms:.3f}ms)")
                print(f"  ✓ Kyber768: OK ({t_kyber_dec_ms:.3f}ms)")
    ...
    def print_summary(self):
        ...
        if self.t_sig_list:
            show_stats(f"Signature Verification ({self.sig_algo_name})", self.t_sig_list)
        if self.t_kyber_list:
            show_stats("Kyber768 Decapsulation", self.t_kyber_list)
        if self.t_aes_list:
        ...
        if self.t_kyber_enc_list:
            show_stats("Kyber768 Encapsulation", self.t_kyber_enc_list)
        if self.t_aes_enc_list:
            show_stats("AES-256 Encryption", self.t_aes_enc_list)
        if self.t_sign_enc_list:
            show_stats(f"Signature Generation ({self.sig_algo_name})", self.t_sign_enc_list)
        # Message sizes
        total_sizes = [
            c + e + s
    ...
    def start(self):
        ...
        print(f"[CONFIG] Crypto: Kyber768 + {self.sig_algo_name}")
        print("=" * 80)
        ...
        # Publish initial key
        self.publish_public_keys()
        print("[MQTT] Published initial Kyber768 public key")
