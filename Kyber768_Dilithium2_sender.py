...
DHT11_GPIO_PIN = board.D4  # GPIO 4 (Physical Pin 7)
LM35_ADC_CHANNEL = 0       # ADS1115 Channel A0 (use 0, not ADS.P0)
READING_INTERVAL = 3       # Seconds between readings
class PQCSender:
    def __init__(self):
        print("=" * 80)
        ...
        print("\n[SENDER] Selecting signature algorithm...")
        ...
        print("[SENDER] Generating signature keypair...")
        self.sender_sig_public_key = self.sig.generate_keypair()
        self.sender_sig_secret_key = self.sig.export_secret_key()
        print(f"[SENDER] ✓ Signature keypair generated ({len(self.sender_sig_public_key)} bytes)")
        # ===== MQTT CLIENT SETUP =====
        print("\n[SENDER] Setting up MQTT client...")
        self.client = mqtt.Client(client_id="pqc_sender_rpi4")
    ...
    def read_dht11_data(self):
        ...
        try:
            ...
            # DHT sensors often throw RuntimeError on read failures
            # This is normal, just retry next cycle
            return None, None
        except Exception as e:
    ...
    def encrypt_and_send_data(self, lm35_temp, dht11_temp, dht11_humidity):
        ...
        try:
            ...
            print(
                ...
                f"Kyber={t_kyber_enc_ms:.2f}ms AES={t_aes_enc_ms:.2f}ms "
                f"Sig={t_sign_ms:.2f}ms"
            )
        except Exception as e:
        ...
        try:
            while True:
                ...
                if lm35_temp is not None or dht11_temp is not None:
                    ...
                    failed_reads = 0  # Reset failure counter
                else:
                    failed_reads += 1
