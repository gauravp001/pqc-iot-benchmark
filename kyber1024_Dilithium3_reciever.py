...
"""
PQC IoT Receiver
Crypto: Kyber1024 + Dilithium3 + AES-256-CTR
"""
import oqs
...
class PQCReceiver:
    def __init__(self):
        ...
        print("[RECEIVER] Configuration: Kyber1024 + Dilithium3")
        print("=" * 80)
        # ===== KYBER1024 INITIALIZATION =====
        print("\n[RECEIVER] Initializing Kyber1024 KEM...")
        self.kem = oqs.KeyEncapsulation("Kyber1024")
        self.kyber_public_key = None
        self.kyber_secret_key = None
        print("[RECEIVER] ✓ Kyber1024 ready")
        # ===== DILITHIUM3 SIGNATURE =====
        print("\n[RECEIVER] Initializing Dilithium3 signature...")
        self.sig = oqs.Signature("Dilithium3")
        self.sig_algo_name = "Dilithium3"
        print("[RECEIVER] ✓ Dilithium3 ready")
        ...
        self.client = mqtt.Client(client_id="pqc_receiver_k1024_d3")
    ...
    def generate_keypairs(self):
        ...
        print("\n[RECEIVER] Generating Kyber1024 keypair...")
        self.kyber_public_key = self.kem.generate_keypair()
        self.kyber_secret_key = self.kem.export_secret_key()
        print(f"[RECEIVER] ✓ Kyber1024 keypair generated ({len(self.kyber_public_key)} bytes)")
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            if msg.topic == TOPIC_ENCRYPTED_DATA:
                ...
                print(f"{'='*80}")
                print(f"  ✓ Dilithium3: VERIFIED ({t_sig_ms:.3f}ms)")
                print(f"  ✓ Kyber1024: OK ({t_kyber_dec_ms:.3f}ms)")
    ...
    def print_summary(self):
        ...
        if n == 0:
            ...
            return
        print("\n" + "=" * 80)
        print("PERFORMANCE SUMMARY - Kyber1024 + Dilithium3")
        print("=" * 80)
        # Calculate duration
        ...
        if self.t_sig_list:
            show_stats("Signature Verification (Dilithium3)", self.t_sig_list)
        if self.t_kyber_list:
            show_stats("Kyber1024 Decapsulation", self.t_kyber_list)
        if self.t_aes_list:
        ...
        if self.t_kyber_enc_list:
            show_stats("Kyber1024 Encapsulation", self.t_kyber_enc_list)
        if self.t_aes_enc_list:
            show_stats("AES-256 Encryption", self.t_aes_enc_list)
        if self.t_sign_enc_list:
            show_stats("Signature Generation (Dilithium3)", self.t_sign_enc_list)
        # Message sizes
        total_sizes = [
            c + e + s
    ...
    def start(self):
        ...
        print(f"[CONFIG] Crypto: Kyber1024 + Dilithium3 + AES-256-CTR")
        print("=" * 80)
        ...
        # Publish initial key
        self.publish_public_keys()
        print("[MQTT] Published initial Kyber1024 public key")
